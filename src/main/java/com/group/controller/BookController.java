...
public class BookController {
   ...
   @ResponseBody
   public String borrow( Borrowbooks borrowbooks){
       Integer bookid = borrowbooks.getBookid();
       Integer userid = borrowbooks.getUserid();
...
//添加图书跳转页面
    ...
    @ResponseBody
    public String evaluation(Evaluation evaluation){
      int c= ev.insrte(evaluation);
        return "提交成功";
    }
    //用户查看评价
    @RequestMapping(value = "evaluationd.do")
    public String evaup(ModelMap map){
        Integer userid=1;
    ...
    public String evaluationform(ModelMap map) throws ParseException {
        Integer userid=1;
